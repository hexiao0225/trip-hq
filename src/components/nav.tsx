...
import { TripSwitcher, type TripLink } from "@/components/trip-switcher";

/**
 * Built for a phone first. The wordmark is gone: the trip's own name is what
 * you need to see at a glance once there is more than one of them, and it
 * doubles as the switcher. Every target is at least 44px tall.
 */
export function Nav({
  trip,
  trips,
  ...
  /** The trip being viewed, or null on the cross-trip pages. */
  trip: TripLink | null;
  trips: TripLink[];
  pendingCount: number;
}) {
  return (
    <header className="sticky top-0 z-20 border-b border-edge bg-background/90 backdrop-blur">
      <div className="mx-auto flex max-w-3xl items-center gap-1 px-3 py-2 sm:px-4">
        {trip ? (
          <TripSwitcher current={trip} trips={trips} />
        ) : (
          <Link
            href="/trips"
        ...
        {trip && (
          <>
            <Link
              href={`/t/${trip.slug}/map`}
              // The map route pulls Leaflet and then map tiles. Not prefetching
              // it keeps that off the wire until the tab is actually opened,
              // which matters on roaming data.
              prefetch={false}
